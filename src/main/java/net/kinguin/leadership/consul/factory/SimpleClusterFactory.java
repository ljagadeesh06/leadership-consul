...
public class SimpleClusterFactory extends AbstractClusterFactory {
    ...
    private static final Logger log = Logger.getLogger(SimpleClusterFactory.class.getName());

    public SimpleClusterFactory() {}
    ...
    public SimpleClusterFactory mode(String mode) {
        this.mode = mode;
        return this;
    }

    public SimpleClusterFactory configure(ClusterConfiguration config) {
        this.config = config;
        return this;
    }

    public SimpleClusterFactory session(SessionConsulClient sessionConsulClient) {
        this.sessionConsulClient = sessionConsulClient;
        return this;
    }

    public SimpleClusterFactory session(String sessionId, int sessionTtlSeconds) {
        this.sessionId = sessionId;
        this.config.getSession().setTtl(sessionTtlSeconds);
        return this;
    }

    public SimpleClusterFactory debug(boolean debug) {
        this.debug = debug;
        return this;
    }
    ...
    public Gambler build() {
        ...
        executor.scheduleAtFixedRate(new UpkeepSession(sessionId, sessionConsulClient), 0, config.getSession().getRefresh(), TimeUnit.SECONDS);

        verbose(String.format("Session refresh scheduled on %s seconds frequency ", config.getSession().getRefresh()));

        if (null == keyValueConsulClient) {
            verbose(String.format("KeyValueConsulClient not specified - default used host %s and port %s", config.getConsul().getHost(), config.getConsul().getPort()));
            keyValueConsulClient = new KeyValueConsulClient(config.getConsul().getHost(), config.getConsul().getPort());
        }

        ActiveGambler gambler = new ActiveGambler(keyValueConsulClient, sessionId, config);
        executor.scheduleAtFixedRate(gambler, config.getElection().getFrequency(), config.getElection().getFrequency(), TimeUnit.SECONDS);
    ...
    private String createAndGetSessionId() {
        ...
        Future<String> sessionId = executor.submit(new CreateSession(this.config.getSession().getTtl(), sessionConsulClient));
        executor.shutdown();
